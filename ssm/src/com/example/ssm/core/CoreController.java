                ...
                for (Parameter parameter : method.getParameters()) {
                    ...
                    switch (parameter.getType().getName()) {
                        ...
                        case"java.lang.Double":
                            if (req.getParameter(name) != null) {
                                valuesParameter[index] = Double.parseDouble(req.getParameter(name));
                                break;
                            }
//                            valuesParameter[index] = 0.0;
                            break;
                        ...
                        case "javax.servlet.http.HttpServletRequest":
                            ...
                            break;
                        case "avax.servlet.http.HttpServletResponse":
                            valuesParameter[index] = resp;
                            break;
                        default:
    ...
    private void review(HttpServletRequest req, HttpServletResponse resp, String result) throws IOException {
        ...
        if (result != null) {
            ...
            switch (s) {
                case "html" -> this.processTemplate(result.substring(i + 1), req, resp);
                case "redirect" -> resp.sendRedirect(result.substring(i + 1));
                case "json" -> {
                    resp.setContentType("application/json,charset=utf-8");
                    resp.getWriter().write(result.substring(i + 1));
                }
                default -> throw new RuntimeException("页面转发无法执行！");
            }
        } else {
    ...
    static class UnableFindException extends RuntimeException {
        public UnableFindException(String s) {
            super(s);
        }
    }
}
